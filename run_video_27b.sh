...
#   run_video_27b.sh 27B 는 bf16 55.6GB 라 GPU 1장(24GB)에 안 들어간다.
...
#   --eval 을 주면 results/<YYYYMMDD_HHMMSS>_video27b_eval/ 로, run_eval.sh 와
...
# --eval 일 때만 쓴다. run_eval.sh 와 같은 기본값 원칙 - 여기서 값을 들고
...
usage() {
  cat <<'USAGE'
Usage: bash run_video_27b.sh [options]
...
Options (환경변수로도 지정 가능 - 명령행이 우선):
  ...
  --eval                   run_eval.sh 처럼 라벨 있는 클립만 처리하고     [EVAL_MODE=1]
                           evaluate_labels.py 로 evaluation.log 를 남긴다.
                           --limit-clips 와 함께 쓸 수 없다(라벨 클립 전체를
                           대상으로 함).
  --labels PATH             --eval 용 정답 라벨 json                    [LABELS]
                           (기본: config.py 의 LABELS_JSON)
  --eval-only DIR           추론을 건너뛰고 그 폴더의 결과만 채점한다
  -h, --help                이 도움말

Examples:
  bash run_video_27b.sh --limit-clips 20         # 마이닝 시험
  bash run_video_27b.sh --gpus 0,1,2,3           # 여유있게 4장에 나눠 올림
  bash run_video_27b.sh --eval --use-egomotion   # 115개 라벨 클립 평가
  bash run_video_27b.sh --eval-only results/20260824_..._video27b_eval
USAGE
}
...
# 추론을 건너뛰고 기존 결과만 채점하는 경로 (run_eval.sh 와 동일)
if [ -n "${EVAL_ONLY:-}" ]; then
...
# --eval: run_eval.sh 와 같은 방식 - 라벨된 uuid 만 --only-uuids 로 골라
